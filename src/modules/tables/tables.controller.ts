...
export class TablesController {
  ...
  @ApiOperation({
    ...
    description:
      'Validates QR token, automatically creates/joins session, and returns table context with session token. Token must be sent via x-qr-token header (v2.0).',
  })
  @ApiHeader({
    name: 'x-qr-token',
    required: true,
  ...
  @ApiResponse({
    status: 200,
    description: 'Returns table context with session token for valid QR token',
  })
  @ApiResponse({
    status: 403,
    description: 'QR token missing or invalid',
  })
  async verifyQrToken(@Req() request: any) {
    const { qrContext, user } = request;

    // Automatically create or join session when verifying QR token
    // Pass customerId if user is authenticated
    const customerId = user?.id || user?.sub || undefined;

    // Generate device ID for this request
    const deviceId = `device_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;

    // Auto-create or join session
    const sessionResult = await this.tablesService.ensureSession(
      qrContext.tableId,
      qrContext.tenantId,
      {
        device_id: deviceId,
  ...
  async startSession(@Req() request: any, @Body() startSessionDto: any) {
    ...
    return this.tablesService.startSession(
      ...
      {
        ...startSessionDto,
        customerId, // Link session to authenticated user
      },
    );
  }
